...
from wares.hetero_score_card.score_card_transform_base import SCORE_CARD_WARE_ID, SCORE_CARD_REPORT

from wares.hetero_score_card.score_card_guest_evaluation_predict import ScoreCardTransformGuestPredict
from wares.hetero_score_card.score_card_host_evaluation_predict import ScoreCardTransformHostPredict
from wares.hetero_score_card.score_card_nodata_evaluation_predict import ScoreCardTransformNodataPredict

score_card_app_new = Blueprint('score_predict_card_app_predict', __name__, static_folder='h', static_url_path='/h')
...
@score_card_app_new.route('/model_report', methods=['POST'])  # model_report
def model_report():
    """模型报告"""
...
class ScoreCardTransformEvaluationPredictWare(BaseWare):

    def __init__(self, setting_file: str):
        # 设置角色字典，注意是类型不是对象
        roles = {
            Role.GUEST: ScoreCardTransformGuestPredict,
            Role.HOST: ScoreCardTransformHostPredict,
            Role.NODATA: ScoreCardTransformNodataPredict
        }
